...
<?php
function insertUserAPI($firstName, $lastName, $emailNew, $image){
  $lastUser_id = 0;
  $connection = new MongoClient();
  // Connection URL
  console.log("Connected successfully to server");
  // Get the documents collection
  $collectionSearch = $connection->selectDB('users');
  $emailQuery = array('email' => emailNew);
  $emailResults = $collectionSearch->find($emailQuery);
  db.users.find({user_id: toString(lastUser_id)});
  if (db.users.find({email: emailNew}) != null)
  {
    collectionSearch = db.users.find({email: emailNew});
    return collectionSearch.user_id;
